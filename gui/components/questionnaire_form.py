...
class QuestionnaireForm(QScrollArea):
    ...
    def _build_section_card(self, sid: str, title: str, questions: list) -> QWidget:
        ...
        for q in questions:
            vbox.addWidget(self._build_question(q))
...
class QuestionnaireForm(QScrollArea):
    ...
    def load_answers(self, answers: dict) -> None:
        """Populate form with existing answers (e.g. when loading a saved use case)."""
        self._answers = dict(answers)
        self._apply_answers_to_widgets()
    ...
    def _build_form(self) -> None:
        ...
        for section_def in SCHEMA["sections"]:
            ...
            box = QGroupBox(f"Section {sid}: {section_def['title']}")
            box.setStyleSheet(
            ...
            for q in qs:
                inner.addWidget(self._build_question(q))
    ...
    def _build_question(self, q: dict) -> QWidget:
        ...
        if q.get("tooltip"):
            ...
            tip_lbl.setToolTip(q["tooltip"])
            tip_lbl.setStyleSheet(
                "color: white; background: #6b7280; border-radius: 8px; "
                "padding: 0 5px; font-size: 11px; font-weight: bold;"
            )
            tip_lbl.setFixedWidth(18)
            title_row.addWidget(tip_lbl)
        title_row.addStretch()
        vbox.addLayout(title_row)
    ...
    def _build_input(self, q: dict) -> tuple[QWidget, QWidget | None]:
        ...
        if qtype == "select":
            ...
            w.currentIndexChanged.connect(lambda _, _w=w, _id=qid: self._on_field_changed(_id, _w.currentData()))
            return w, follow_up_widget
        ...
        # Follow-up types handled here too (used standalone for some questions)
        return QWidget(), None
    ...
    def _build_follow_up(self, follow_up: dict) -> QWidget:
        ...
        frame = QFrame()
        frame.setStyleSheet(
            "QFrame { border-left: 3px solid #d1d5db; padding-left: 10px; margin-left: 4px; }"
        )
        vbox = QVBoxLayout(frame)
        vbox.setContentsMargins(8, 4, 0, 4)
        vbox.setSpacing(4)
        ...
        if ftype == "text":
            ...
            for opt in fq.get("options", []):
                w.addItem(opt["label"], opt["value"])
            self._widgets[fid] = w
            w.currentIndexChanged.connect(lambda _, _w=w, _id=fid: self._on_field_changed(_id, _w.currentData()))
            vbox.addWidget(w)
